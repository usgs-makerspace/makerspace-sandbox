...
create_maps <- function(data_downloaded, conus_sf, canada_sf, mexico_sf) {
  ...
  for(i in 1:length(dates)) {
    ...
    map_fns[i] <- make_map(dates[i], conus_sf, canada_sf, mexico_sf, map_data_i)
  }
...
make_map <- function(date, conus_sf, canada_sf, mexico_sf, map_data) {
  ...
  conus_data_sf <- left_join(conus_sf, map_data, by = "hru_id_nat") %>% 
    filter(!is.na(value)) %>% # there are some HRUs in conus_sf that aren't in the data
  ...
  plot(st_geometry(conus_data_sf), col = conus_data_sf$color, border=NA, 
...
load_conus_shape <- function(fn, proj) {
  
  topo_data_conus <- topojson_read(fn)
  conus_sf <- st_as_sf(topo_data_conus)
  st_crs(conus_sf) <- proj
  ...
  # conus_sf_valid <- st_make_valid(conus_sf) # Needs pkg `lwgeom`
