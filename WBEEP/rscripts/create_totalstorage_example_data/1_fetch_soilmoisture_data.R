...
# Download and unzip: nhru_soil_moist_tot.zip
# Then save the nhru_soil_moist_tot.csv file to the `WBEEP/cache/` folder. 
...
soilm_data[, Year := as.numeric(format(Date, "%Y"))] # use data table to add year column
max_date <- max(soilm_data$Date) # get the max date in the dataset to use for filtering
max_year <- max(soilm_data$Year) # get the max year in the dataset to use for filtering
soilm_data_15yrs <- soilm_data[Year >= max_year - 15]
soilm_data_1wk <- soilm_data_15yrs[max_date - Date <= 7]

# Saving these intermediate versions takes ~ 3 min
saveRDS(soilm_data_15yrs, "WBEEP/cache/nhru_soilm_15yrs.rds")
